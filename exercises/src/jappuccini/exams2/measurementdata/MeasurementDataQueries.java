package jappuccini.exams2.measurementdata;
...
/**
 * MeasurementDataQueries
 *
 * @author Daniel Appenmaier
 * @version 1.0
 *
 */
public class MeasurementDataQueries {
  ...
  public static void main(String[] args) {
    ...
    MeasurementData.getData().stream().filter(m -> m.date().getYear() == 2020)
    ...
    OptionalDouble averagePercentage = MeasurementData.getData().stream()
        .filter(m -> m.category() == 'X').mapToInt(m -> m.percentage()).average();
    System.out.println(averagePercentage);
    System.out.println();

    System.out.println(
        "Alle Messdaten, bei denen die Temperatur im Sommer (Juni - August) bei mindestens 30° lag");
    MeasurementData.getData().stream()
        .filter(m -> m.date().getMonthValue() == 6 || m.date().getMonthValue() == 7
            || m.date().getMonthValue() == 8)
        .filter(m -> m.temperature() >= 30).forEach(System.out::println);
    System.out.println();
    ...
    boolean dataExists =
        MeasurementData.getData().stream().anyMatch(m -> m.country().equals("Deutschland")
    ...
    MeasurementData.getData().stream().collect(Collectors.groupingBy(m -> m.country()))
