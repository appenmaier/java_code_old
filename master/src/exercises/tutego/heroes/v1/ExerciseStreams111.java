...
import exercises.tutego.heroes.v1.Heroes.Hero.Sex;
...
public class ExerciseStreams111 {
	...
	public static void main(String[] args) {
		...
		// 2
		System.out.println("Terminal 2: " + heroes.stream().allMatch(t -> t.yearFirstAppearance > 1900));
		...
		// 4
		System.out.println("Terminal 4: "
				+ heroes.stream().min((o1, o2) -> Integer.compare(o1.yearFirstAppearance, o2.yearFirstAppearance)));
		...
		Map<Boolean, List<Hero>> heroesBefore1970 = heroes.stream()
				.collect(Collectors.partitioningBy(t -> t.yearFirstAppearance < 1970));
		...
		// 1
		System.out
				.println("Intermediate 1: " + heroes.stream().filter(t -> t.sex == Sex.FEMALE ? true : false).count());

		// 2
		System.out.println("Intermediate 2: ");
		heroes.stream().sorted((o1, o2) -> Integer.compare(o1.yearFirstAppearance, o2.yearFirstAppearance))
				.forEach(t -> System.out.println(t.toString()));

		// 3a
		System.out.println("Intermediate 3a: ");
		heroes.stream().filter(t -> t.sex == Sex.FEMALE ? true : false).map(t -> t.name).forEach(System.out::println);
		...
		int[] yearFirstAppearances2 = heroes.stream().mapToInt(t -> t.yearFirstAppearance).distinct().toArray();
		System.out.println(Arrays.toString(yearFirstAppearances2));

		// 4
		System.out.println("Intermediate 4: ");
		Heroes.UNIVERSES.stream().flatMap(Heroes.Universe::heroes).map(t -> t.name).forEach(System.out::println);
	}
